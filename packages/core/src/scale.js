...
//
// Used by data sources whose harmony is derived from real frequency/period
// ratios rather than from diatonic modes — e.g. SONDE maps orbital-period
// ratios to consonant intervals ("music of the spheres"). Kept here in core
// because ratio↔pitch conversion is domain-agnostic.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Fold an arbitrary positive ratio into a single octave [1, 2).
 * e.g. 6 → 1.5, 0.25 → 1.0. Preserves the pitch-class while discarding octave.
 */
export function octaveReduce(ratio) {
  if (!(ratio > 0)) return 1;
  let r = ratio;
  while (r >= 2) r /= 2;
  while (r < 1) r *= 2;
...
/**
 * 5-limit just-intonation lattice within one octave [1, 2].
 * Twelve consonant intervals plus the octave, used as a quantization target so
 * that arbitrary real-world ratios snap to harmonically pleasing pitches.
 */
...
/**
 * Snap an arbitrary ratio to the nearest just-intonation interval (octave-aware).
 * Returns the octave-reduced just ratio in [1, 2). Comparison is done in cents
 * so the nearest *perceptual* interval is chosen.
 */
export function quantizeRatioToJust(ratio, lattice = JUST_RATIOS) {
  const target = octaveReduce(ratio);
  const targetCents = ratioToCents(target);
  let best = lattice[0];
  let bestDelta = Infinity;
  for (const r of lattice) {
    const delta = Math.abs(ratioToCents(r) - targetCents);
    if (delta < bestDelta) {
      bestDelta = delta;
      best = r;
    }
  }
  // The octave (2/1) is the same pitch-class as unison once reduced.
  return best >= 2 ? 1 : best;
}

/**
 * Derive a frequency from a base frequency and a ratio, optionally octave-shifted.
 * @param {number} baseFreq - Reference frequency in Hz (the "fundamental").
 * @param {number} ratio - Frequency ratio relative to the base.
 * @param {number} octaveShift - Whole octaves to transpose (can be negative).
 */
export function freqFromRatio(baseFreq, ratio, octaveShift = 0) {
  return baseFreq * ratio * Math.pow(2, octaveShift);
}
